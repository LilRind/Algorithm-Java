    ...
    public static void main(String[] args) {
        ...
        while (l < r) {
            ...
            if (check(s, k, mid, n)) { // k个字串，权值最大的那个子串的权值不超过 mid
                r = mid; // 如果满足条件,说明mid比目标值大，说明在[l，mid)，则更新最大权值为中间值
            } else {
...
//
//    // 判断当前权值x是否满足 count >= k
//    public static boolean check(String s, int x, int k){
//        HashSet set = new HashSet();
//        int count = 0;
//        for (int i = 0; i<s.length(); i++){
//            int j = i;
//            while ((j-i+1) * set.size() <= x && j<s.length()){
